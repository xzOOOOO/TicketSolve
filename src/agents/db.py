...
class DBAgent(BaseAgent):
    ...
    async def run(self, state: SystemState) -> dict:
        ...
        try:
            ...
            # ========== 通过通信总线发送协议消息 ==========
            # 只有当 communication_bus 不为 None 时才进入协作模式
            if self.bus:
                agent_messages = []
                ...
                hypothesis_messages = self.bus.publish_hypothesis(
                    sender=self.name,
                    content=f"数据库诊断假设: {result_dict.get('hypothesis') or result_dict.get('diagnosis')}",
                ...
                # 与 hypothesis 的区别：hypothesis 是 "我认为可能是 X"，diagnosis 是 "我确认是 X"
                ...
                # 例如：db_agent 需要 net_agent 确认 "nginx 入口路由是否正常"
                for request in collaboration_requests:
                    target = request["target_agent"]
                    # 白名单校验：只允许向真实存在的 Agent 发请求，防止 LLM  hallucinate（编造）Agent 名字
                    if target in _VALID_AGENTS and target != self.name:
                        request_messages = self.bus.request_evidence(
                            ...
                            reason=request.get("reason") or f"数据库诊断需要 {target} 补充证据",
                ...
                # 把所有协议消息合并到状态更新中
                # LangGraph 会通过 operator.add 把这些消息追加到 state.agent_messages
                if agent_messages:
