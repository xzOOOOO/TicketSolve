...
本模块只保留网络领域的差异化配置，通用诊断流程由 DiagnosticAgent 提供。
"""

# BaseChatModel：LangChain 聊天模型基类，用于诊断推理
from langchain_core.language_models import BaseChatModel
# BaseTool：LangChain 工具基类，用于注入网络诊断工具
...
# NET_DIAGNOSIS_PROMPT：网络诊断结论生成 Prompt
# NET_PROMPT：网络工具调用阶段 Prompt
from prompts import NET_DIAGNOSIS_PROMPT, NET_PROMPT


class NetAgent(DiagnosticAgent):
    """
    网络诊断专家 Agent。

    用途：
        使用网络相关 MCP 工具诊断 DNS、入口路由、端口连通性等故障。
    """
